...
В доступі є макетні плати на основі ESP32 від різних виробників і їх якість, відповідно, теж буває різною — при плануванні серійного виробництва це варто перевіряти наперед.
...
Інструментарій:
* стандартні Linux/Windows IDE (VS Code, IntelliJ тощо) і тулчейни (C/C++, Rust, Golang, Python чи будь-що инше, аби там були бібліотеки для необхідних апаратних інтерфейсів — GPIO тощо).

Для роботи Raspberry Pi до ціни слід додати ціну SD-картки — досить простенької картки на 16Gb (краще, звісно, 32), але якщо потрібно зберігати на картці відео, то слід брати картку з високою швидкодією (дешеві — не справляються з відеопотоком). І, якщо потрібні серйозні обчислення, копійчину на вентилятор і/або набір радіаторів (з мого досвіду — на RPI4 досить радіяторів, набір коштує 20 грн.) Для повноцінного використання приладу потрібен корпус — стандартні корпуси для RPI4 можна купити за 100-200 грн. 

Якщо на Raspberry Pi настроїти SSH доступ і вхід по WiFi у свою мережу (це можна зробити ще до першого запуску, конфігуруючи оперсистему перед її записом на SD-картку), то можна працювати з нею по SSH, приєднавши лиш живлення. Якщо ж приєднати монітор-клавіятуру-мишку — це буде звичайний Linux-компʼютер. В обох випадках програми для Raspberry Pi білдяться на самому Raspberry Pi і можуть бути там відладжені в звичному консольному режимі (або й з дебагером) — це зручніше, ніж білдити програми для мікроконтролерів на ПК, завантажувати їх на мікроконтролер, а потім рішати проблеми з відладкою (для цього часом потрібно піднімати непрості дебаг-інструменти).

Важливо. Тактова частота Raspberry Pi знаходиться на рівні частот сучасних PC (Raspberry Pi 5 — 2.4 ГГц), але це 32-розрядна ARM-архітектура, яка при рівній частоті дає продуктивність приблизно в 2-3 рази нижчу, ніж 64-розрядні процесори на ПК з архітектурою x86_64 (я проводив відповідні тести на задачі, яка потребує виключно процесорних ресурсів, такий же результат видно і при виконанні коду з використанням OpenCV). 
...
Вона, однак, істотно уступає за параметрами наступній платі від STM, яка при цьому ненабагато дорожча. Також на Blue Pill boot-режим активується не кнопкою, а перемичкою — це доволі незручно.
...
В цілому, лінійка китайських мікрокомпʼютерів Orange Pi помітно дешевша ніж Raspberry Pi (при рівних параметрах), але й викликає більше нарікань (але при цьому вони досить популярні у нас — отже, це дійсно робочі рішення). Наприклад, на процесорах Orange Pi трапляються проблеми з перегрівом, але в гуглі є досить рекомендацій, як вирішити їх незначним пониженням тактової частоти.


### Raspberry Pi 4 1Gb — 2199 грн. — https://minicomp.com.ua/ua/raspberry-pi/raspberry-pi-4/raspberry-pi-4-model-b-1gb

Це вже повноцінний компʼютер (якщо приєднати монітор, клавіятуру і мишку), на якому можна ганяти задачі, які потребують високої продуктивности (4 ядра по 1.5/1.8 ГГц).
