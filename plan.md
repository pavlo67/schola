...
B.5. Інтерфейси, поліморфізм, наслідування. Composition over inheritance. "Види інтерфейсів" в Go. Pros and cons.

B.6. Архітектура системи: Model-View-Controller. Patterns (шаблони, зразки, моделі).

B.7. Модульна/пакетна структура системи: публічні репозиторії і каталоги (NPM), внутрішня структура, Dependency Injection.

B.8. Контроль вхідних параметрів. Контроль типів і структур в нетипізованій мові. JS: if (str == "undefined")...

B.9. Параметризація коду - "magic numbers", константи, конфіги. 

B.10. Юніт-тести та інтегральне тестування (ціна помилки: хвилина при компіляції - година при тестуванні - день при відладці). Ручне тестування, API-тестування. Тести — підтверджують помилки, але, принципово, не можуть підтвердити їх відсутність. Помилки є скрізь.

B.11. Документація та коментування (зокрема: історія RFC, коментарі в Go, doc-strings в JS/Python). Кращий коментар — добрий код. Але — необхідне коментування мотивів і девіяцій.

B.12. Типічні помилки в розробці і критерії оцінки: швидкість vs. надійність :-); "майже робоча система"; читабельність і красота коду; стабільність системи і легкість підтримки-відладки... 

B.13. KISS. YAGNI. If it ain't broke, don't fix it.

B.14. Життєвий цикл розробки і підтримки. Формулювання вимог, дизайни. Agile, scrum, waterfall, kanban... PoC, MVC, alpha-beta releases, release-candidate, release. Оточення: develop, test, stage, production. Тести, контроль цілісности, аналіз невдач ("post mortem").
...
C.2. Функції роботи зі списками: map, grep/filter, sum, zip, list comprehension... "Лямбда-вирази".

C.3. Стек (LIFO) і черга (FIFO); алгоритм двох стеків Дейкстри; callback-цикл в Node.js. Стек повернень і стек даних.

C.4. Структура vs. асоціятивний масив (Go map, JS object, Python dict, Perl hash). Швидкодія!

C.5. Деревовидні структури і, на загал, графи. Рекурсія.

C.6. Значення і вказівники. Передача параметрів по вказівнику (посиланню) і по значенню. Обʼєкти-"константи" (зокрема, в JS). 

C.7. Виклик функції — що під капотом. Процедурна структура коду, колбеки/проміси/async-await. 

C.8. Класи/обʼєкти, замикання (closures).

C.9. Рядки і регулярні вирази.

C.10. Типічні проблеми: вічний цикл, вічна рекурсія, memory leaks, +-1 на індексі циклу, декларування, області видимости і змішування імен змінних (JS: hoisting, "let" vs. "var").

C.11. Одно- і багатопоточний код. Thread-safe functions. Dead lock, data race. Мʼютекси і критичні секції, канали.

C.12. Представлення часу: проблеми з часовими поясами (Clickhouse, датасети) vs. unix timestamp.

C.13. Пошук: перебір vs. бінарний пошук vs. пошук по індексу vs. ще якось? оцінки швидкости. 

C.14. Сортування: бульбашкове vs. сортування вставкою vs. швидке vs. сортування злиттям vs. ще якось? оцінки швидкости. 

C.15. Індексація: збалансовані дерева (B-tree, червоно-чорні дерева), ще щось? оцінки швидкости. 

C.16. Архітектура БД: схема, індекси, foreign keys, нормалізація-денормалізація, тригери.

C.17. Архітектура системи: моноліт - мікросервіси. Внутрішні протоколи звʼязку.

C.18. Загальне: ієрархія (зокрема: фронт-бек, стек мережевих протоколів...), швидкодія vs. необхідний обʼєм памʼяти, швидкість розробки vs. ціна підтримки.
...
E.1. Web-сервер. Розробка сервера для цих курсів.
